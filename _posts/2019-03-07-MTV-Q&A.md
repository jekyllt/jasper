...
>>>>>>> e8d4fb7d4b813bcbad3599f46d0902ae77dc19d2
author: roseline
categories: roseline
---
...
class ContactView(FormView):
    ...
    form_class = ContactForm #forms.py에 정의한 클래스 명
...
[장고 공식 문서 참고 : form mixin 클래스의 구성](https://docs.djangoproject.com/en/1.10/ref/class-based-views/mixins-editing/#django.views.generic.edit.FormMixin)
...
URL 이름을 \<myapp>_detail 등의 방법으로 부를 때 생기는 또 다른 문제는 이 \<myapp> 부분이 서로 겹칠 때 벌어진다. 이런 경우 URL 이름 공간을 통해 이를 간단히 해결할 수 있다.
...
#contactapp의 urls.py
urlpatterns = [
    path('report_problem/', views.get_problem, name='report'),
]

#contactmonger의 urls.py
urlpatterns = [
    path('get_contact/', views.get_contact, name='create'),
]
</code></pre>
